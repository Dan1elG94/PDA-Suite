...
`;
        ...
        function fitHeight() {
            ...
            try {
                ...
                const sekciaZoznamu = sc.closest('section');
                // pevne pripnuty lavy stlpec (modul fullLeft) sa s obsahom neposuva - posun sa nepripocita
                const posunZoznamu = sekciaZoznamu && !sc.closest('.pda-layout-left') ? sekciaZoznamu.scrollTop : 0;
                if (box && scR.top + posunZoznamu > 0) {
                    ...
                    // rezerva 8 px je vacsia nez prah zmeny vysky (6 px), takze obsah nikdy nepretecie
                    const spodok = dno - Math.max(OKRAJ_3J, sucet + 8);
                    // horny okraj zoznamu bez posunu stranky - vysledok nezavisi od toho, ci je posunuta
                    const horeZoznamu = scR.top + posunZoznamu;
...
`;
        ...
        /*
         * Je prave zobrazeny pohlad uvodu (Main)? Zistuje sa len z DOM - skryta stranka
         * appky ma triedu sapMNavItemHidden / display:none. getCurrentPage() sa zamerne
         * nevola: pred prvym vykreslenim appky by si UI5 pri nom zalozilo zoznam stranok
         * a mohlo by tak ovplyvnit, ktora stranka sa otvori ako prva (3.2.15).
         */
        function naUvode() {
            const m = document.getElementById('Main');
            if (!m) return false;
            if (m.classList.contains('sapMNavItemHidden')) return false;
...
/* ================= STAVOVE TLACIDLA: vyraznejsie obrazky v pozadi (3.2.13, 3.2.14) =================
   Obrazky v pozadi (Stretnutia, Prestavka, Cakanie, Vyroba, Udrzba, ...): priehladnost
   povodne .5, v 3.2.13 .65, v 3.2.14 plna sila 1 (zmiesanie soft-light ostava). Silu
   meni jedina hodnota --pda-obraz-sila; dalsi stupen vyraznosti by bolo zmiesanie
   overlay (napr. .75). Tlacidlo samotne sa nemeni. */
${B} { --pda-obraz-sila:1; --pda-obraz-zmes:soft-light; }
${B} .statusBtn[data-pda-obraz]::after { opacity:var(--pda-obraz-sila) !important; mix-blend-mode:var(--pda-obraz-zmes) !important; }
...
/* Osobny stav ma obycajnu hlavicku (cela je tlacidlo na zbalenie) - klik na nu sa
   nesmie dostat k UI5, inak by panel vnutorne zbalilo (3.2.16). Hlavicka ma len nadpis. */
${B} #Main--Tasks_Panel > .sapMPanelWrappingDiv,
${B} #WorkcenterDetail--Tasks_Panel > .sapMPanelWrappingDiv { pointer-events:none !important; cursor:default !important; }
...
/* neaktivne tlacidlo (napr. BOM bez komponentov) sa nehybe a nema silny tien (3.2.16) */
${B} #__pda_detail_rightcol__ #__pda_order_drawing_button__:disabled,
${B} #__pda_detail_rightcol__ #WorkcenterDetail--BoM_Button.sapMBtnDisabled,
${B} #__pda_detail_rightcol__ .nd-bom.sapMBtnDisabled { transform:none !important; }
${B} #__pda_detail_rightcol__ #__pda_order_drawing_button__:disabled,
${B} #__pda_detail_rightcol__ #WorkcenterDetail--BoM_Button.sapMBtnDisabled .sapMBtnInner,
${B} #__pda_detail_rightcol__ .nd-bom.sapMBtnDisabled .sapMBtnInner { box-shadow:0 2px 8px rgba(16,36,63,.06) !important; }
/* ================= NACITAVANIE: jeden velky indikator v strede obrazovky (3.2.14) =================
   Appka pri nacitani dat ukaze v kazdej karte vlastny cakaci indikator (tri znacky S).
   Tie v kartach su skryte - jemny zavoj karty pocas nacitania ostava - a namiesto nich
   je jeden, 3x vacsi, v strede obrazovky (JS nacitanie(), tie iste triedy UI5, takze
   aj ten isty obrazok S od Simplifiera). Okna a rozbalovacie okienka si svoj indikator
   nechavaju - stredovy by lezal pod nimi. Globalny indikator appky sa nemeni. */
${B} .sapUiLocalBusyIndicator:not(.pda-3j-busy):not(.sapMDialog *):not(.sapMPopover *):not(#sapUiBusyIndicator *) > .sapUiLocalBusyIndicatorAnimation {
  visibility:hidden !important; }
#__pda_3j_nacitanie__ { position:fixed; left:50%; top:50%; width:0; height:0; z-index:9; pointer-events:none; }
#__pda_3j_nacitanie__ > .sapUiLocalBusyIndicator { position:absolute !important; left:0 !important; top:0 !important;
  right:auto !important; bottom:auto !important; width:0 !important; height:0 !important;
  background:transparent !important; border-radius:0 !important; opacity:1 !important; }
`;
            document.head.appendChild(st);
        }
        ...
        /*
         * Nacitavanie (3.2.14): kym je v stranke aspon jeden viditelny cakaci indikator
         * appky (mimo okien a rozbalovacich okienok), je v strede obrazovky jeden nas,
         * 3x vacsi. Sklada sa z tych istych tried UI5 ako indikator appky, takze ma aj
         * ten isty obrazok. Velkost sa berie z indikatora appky (jeho font-size x 3).
         */
        const NACITANIE_ID = '__pda_3j_nacitanie__';
        function nacitanie() {
            let aktivny = null;
            document.querySelectorAll('.sapUiLocalBusyIndicator').forEach((b) => {
                if (aktivny || b.classList.contains('pda-3j-busy') || b.classList.contains('sapUiHiddenBusyIndicatorAnimation')) return;
                if (b.closest('.sapMDialog, .sapMPopover, #sapUiBusyIndicator')) return;
                const r = b.getBoundingClientRect();
                if (r.width > 0 && r.height > 0) aktivny = b;
            });
            let el = document.getElementById(NACITANIE_ID);
            if (!aktivny) {
            ...
            if (!el) {
                ...
                obal.className = 'sapUiLocalBusyIndicator sapUiLocalBusyIndicatorFade pda-3j-busy';
                const anim = document.createElement('div');
                anim.className = 'sapUiLocalBusyIndicatorAnimation sapUiLocalBusyIndicatorAnimStandard';
                for (let i = 0; i < 3; i++) anim.appendChild(document.createElement('div'));
                obal.appendChild(anim);
                el.appendChild(obal);
                document.body.appendChild(el);
            }
            const zaklad = parseFloat(getComputedStyle(aktivny).fontSize) || 32;
            const velkost = Math.round(zaklad * 3) + 'px';
            const obal = el.firstChild;
            if (obal && obal.style.fontSize !== velkost) obal.style.fontSize = velkost;
        ...
        function apply() {
            injectStyles();
            [priehladnePanely, patka, farbyPrehladu, oznacPrihlasenie, kolace3D, nadpisPracoviska, nacitanie].forEach((f) => {
